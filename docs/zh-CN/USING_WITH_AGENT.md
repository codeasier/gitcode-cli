...
agent 应先确认在 GitCode 仓库之外执行 `gh --version` 仍会调用原生 GitHub CLI，再分别在两类仓库中运行安全的只读命令。代理依次按 `GC_GH_TARGET`、显式仓库 host、当前 `origin` 判断目标；无法判断时使用原生 `gh`。目标明确为 GitCode 时，不支持的命令会被拒绝。原生 `gh` 自动查找失败时可设置 `GC_REAL_GH`，使用 `gc setup gh-proxy --uninstall` 可以移除代理。
